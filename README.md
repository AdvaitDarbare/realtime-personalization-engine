...
- **Flink SQL** for continuous stream transformation: aggregations, multi-stream joins, windowing, and upsert sinks that maintain a live state per key.
...
- Active interest category from the most frequent recent category.
...
**Personalization agent tools** (called in order):
...
3. Uses ChromaDB's default local model (`all-MiniLM-L6-v2`) — no API key, runs entirely on your machine. Model is downloaded once (~79MB) and cached at `~/.cache/chroma/`.
...
- Personalization agent: semantic search → live profile lookup → GPT-4o recommendation.
- Merchandising agent: full product scan → GPT-4o promotion recommendations.
- Recommendation output written back to Kafka `recommendations` topic.

Good next improvements:

- Provision a Grafana dashboard automatically instead of configuring it by hand.
